        ...
        <div class="row">
            <div class="col-md-3">
                <div class="card" style="background: #0a0a0a">
                    <div class="card-header" style="color:white; font-weight:bold;">Categories</div>
                    <div class="card-body" >
                        <ul class='list-group' id='cat-list' >
                            <li class='list-group-item <?php echo $cid == 'all' || $cid == 0 ? 'active' : '' ?>' data-id='all' data-href="index.php?page=home&category_id=all">All</li>
                            <?php
                                $cat = $conn->query("SELECT * FROM categories ORDER BY name ASC");
                                while($row = $cat->fetch_assoc()):
                                    $cat_arr[$row['id']] = $row['name'];
                            ?>
                                <li class='list-group-item <?php echo $cid == $row['id'] ? 'active' : '' ?>' data-id='<?php echo $row['id'] ?>' data-href="index.php?page=home&category_id=<?php echo $row['id'] ?>"><?php echo ucwords($row['name']) ?></li>
